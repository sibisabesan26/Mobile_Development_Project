    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        adapter = FeedbackAdapter(
            ...
            onEditClick = { position ->
                // Load feedback back into input fields for editing
                val feedback = feedbackList[position]
                val regex = Regex("^(.*) \\((.*)\\): (.*)$")
                val match = regex.find(feedback)
                if (match != null) {
                    binding.inputName.setText(match.groupValues[1])
                    binding.inputEmail.setText(match.groupValues[2])
                    binding.inputComments.setText(match.groupValues[3])
                }
                // Remove old entry so new submission replaces it
                feedbackList.removeAt(position)
                adapter.notifyItemRemoved(position)
            }
        )
        ...
        binding.btnSubmit.setOnClickListener {
            ...
            if (name.isNotEmpty() && email.isNotEmpty() && comments.isNotEmpty()) {
                val feedback = "$name ($email): $comments"
